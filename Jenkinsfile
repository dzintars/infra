pipeline {
  ...
  stages {
    ...
    stage('Ansible play') {
      steps {
        dir('./ansible') {
          ansiblePlaybook credentialsId: 'ssh-key', becomeUser: 'dzintars', installation: 'ansible', inventory: 'hosts', playbook: 'play/demo.yml', vaultCredentialsId: 'AnsibleVaultPass'
        }
      }
    }
  }
